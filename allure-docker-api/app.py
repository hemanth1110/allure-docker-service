...
def generate_file_path(project_id, build_id, file_type):
    ...
    LOGGER.info('PATH GENERATION - Using custom DMaas path for project: %s', project_id)
    ...
    if product_code == 'ld':  # lens desktop
        # For lens desktop: major.minor.0.build_id (e.g., 2.3.0.1234)
...
def get_notes_file_path(project_id, build_id):
    """Get file path for notes"""
    return generate_file_path(project_id, build_id, 'notes')

def get_jira_file_path(project_id, build_id):
    """Get file path for jira tickets"""
    return generate_file_path(project_id, build_id, 'jira')
...
def handle_get_request(project_id, build_id, get_file_path_fn, data_type):
    """Generic function to handle GET requests (fetch data)"""
    file_path = get_file_path_fn(project_id, build_id)
    
    LOGGER.info('%s GET Operation - Project: %s, Build: %s, Path: %s', 
                data_type.upper(), project_id, build_id, file_path)
...
def handle_post_request(project_id, build_id, data_value, data_key, 
                       get_file_path_fn, data_type, success_message):
    """Generic function to handle POST requests (add data)"""
    file_path = get_file_path_fn(project_id, build_id)
    
    LOGGER.info('%s POST Operation - Project: %s, Build: %s, Data: %s', 
                data_type.upper(), project_id, build_id, data_value)
...
def handle_delete_request(project_id, build_id, index, get_file_path_fn, 
                         data_type, success_message):
    """Generic function to handle DELETE requests (remove data)"""
    file_path = get_file_path_fn(project_id, build_id)
    
    LOGGER.info('%s DELETE Operation - Project: %s, Build: %s, Index: %s', 
...
def get_notes_endpoint(project_id, build_id):
    ...
    try:
        ...
        return handle_get_request(project_id, build_id, get_notes_file_path, 'notes')
    except Exception as ex:
...
def add_note_endpoint(project_id, build_id):
    ...
    try:
        ...
        return handle_post_request(project_id, build_id, note, 'note', 
                                 get_notes_file_path, 'note', 'Note added successfully')
    except Exception as ex:
...
def get_jira_tickets_endpoint(project_id, build_id):
    ...
    try:
        ...
        return handle_get_request(project_id, build_id, get_jira_file_path, 'tickets')
    except Exception as ex:
...
def add_jira_ticket_endpoint(project_id, build_id):
    ...
    try:
        ...
        return handle_post_request(project_id, build_id, ticket_id, 'ticket_id', 
                                 get_jira_file_path, 'ticket', 'Jira ticket added successfully')
    except Exception as ex:
...
def delete_jira_ticket_endpoint(project_id, build_id, index):
    ...
    try:
        ...
        return handle_delete_request(project_id, build_id, index, get_jira_file_path, 
                                   'ticket', 'Jira ticket deleted successfully')
    except Exception as ex:
